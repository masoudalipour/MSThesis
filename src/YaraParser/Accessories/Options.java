...
public class Options implements Serializable {
    ...
    public static void showHelp() {
        ...
        String output = "© Yara YaraParser.Parser \n" +
                ...
                "Usage:\n" +
                "* Train a parser:\n" +
                "\tjava -jar YaraParser.jar train -train-file [train-file] -dev [dev-file] -model [model-file] -punc " +
                "[punc-file]\n" +
                "\t** The model for each iteration is with the pattern [model-file]_iter[iter#]; e.g. mode_iter2\n" +
                "\t** [punc-file]: File contains list of pos tags for punctuations in the treebank, each in one " +
                "line\n" +
                "\t** Other options\n" +
                "\t \t -cluster [cluster-file] Brown cluster file: at most 4096 clusters are supported by the parser " +
                ...
                "\t \t pt:[#partail_training_starting_iteration] (default:3; shows the starting iteration for " +
                ...
                "\tjava -jar YaraParser.jar parse_conll -input [test-file] -out [output-file] -model [model-file] " +
                "nt:[#_of_threads (optional -- default:8)] \n" +
                ...
                "* Parse a tagged file:\n" +
                "\tjava -jar YaraParser.jar parse_tagged -input [test-file] -out [output-file]  -model [model-file] " +
                "nt:[#_of_threads (optional -- default:8)] \n" +
                ...
                "nt:[#_of_threads (optional -- default:8)] \n" +
                "\t** The test file should have the conll 2006 format; each word that does not have a parent, should " +
                "have a -1 parent-index" +
                "\t** Optional: -score [score file] averaged score of each output parse tree in a file\n\n" +
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].equals("--help") || args[i].equals("-h") || args[i].equals("-help"))
                ...
                options.partialTrainingStartingIteration =
                        Integer.parseInt(args[i].substring(args[i].lastIndexOf(":") + 1));
